...
export default function LoginPage() {
  ...
  const handleLogin = () => {
    ...
    if (email.toLowerCase().includes('admin')) {
      login('demo-token', { id:1, name:'Ragah Admin', email, role:'admin', points:1000, level:'Eco-Master' })
      navigate('/admin/dashboard')
    } else {
      login('demo-token', { id:2, name:'Budi Santoso', email, role:'user', points:3850, level:'Penanaman Pohon' })
        ...
        <div className="relative z-10 flex items-center justify-center h-full p-10">
          <div className="w-full max-w-md">
            ...
            <button onClick={handleLogin}
              className="w-full py-4 rounded-full text-white font-bold text-base cursor-pointer mb-4 transition-opacity hover:opacity-90"
              style={{ background:'#1B4332' }}>Log in</button>
